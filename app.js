 ...
 *    2. IndexedDB helpers
 *    3. Crypto (token encrypt/decrypt)
 *    4. State management (load / persist / normalize)
 *    5. Utility functions
 *    6. Cloud pieces (build / apply / migrate)
 *    7. GitHub sync (UP / DOWN) with progress toast & SHA skip
 *    8. GitHub snapshots (save / list / restore)
 *    9. Startup diff check (local vs remote)
 *   10. Auto-save (visibilitychange / pagehide / beforeunload)
 *   11. Text backup & attachment helpers
 *   12. UI rendering (chapters, memos, favorites, analytics, etc.)
 *   13. Editor features (replace, highlight, search)
 *   14. AI integration
 *   15. Download / Export / Import
 *   16. Keyboard shortcuts & event listeners
 * =================================================================== */
...
function computeSHA256Hex(text) {
    // Simple string hash for comparing content changes (not cryptographic)
    let hash = 0;
    const str = text || '';
...
async function migrateLegacyCloudIfNeeded(headers, owner, repo, remote, remoteMeta) {
    ...
    let legacyData = null, legacyAiChat = null;
...
/* ------------------------------------------------------------------
 *  7. GitHub sync with progress toast & SHA-based skip
 * ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------
 *  8. GitHub snapshots (save / list / restore / preview)
 * ------------------------------------------------------------------ */
...
async function checkRemoteDiffOnStartup(token) {
    ...
    // Show diff dialog
    const pieceLabelMap = { settings: '設定', stories: '話データ', memos: 'メモデータ', aiChat: 'AIチャット履歴' };
    const labels = changedPieces.map(k => pieceLabelMap[k] || k);

    showDiffDialog(labels, changedPieces, async (selectedKeys) => {
        if (!selectedKeys.length) return;
        showProgressToast('DOWN: 選択したデータを取得中...', 0, selectedKeys.length);
        const remote = {};
        let step = 0;
        for (const key of selectedKeys) {
            ...
            const url = `https://api.github.com/repos/${owner}/${repo}/contents/${path}`;
            const got = await requestJson(url, { headers });
            if (got.res.ok && got.body?.content) remote[key] = fromBase64ToJson(got.body.content);
            step++;
            showProgressToast('DOWN: 選択したデータを取得中...', step, selectedKeys.length);
        }
        await addLocalSnapshot('before-startup-diff-sync', structuredClone(state));
        applyCloudPieces(remote);
        // Update local syncMeta to avoid re-prompting
        state.syncMeta = remoteMeta;
        await persistNow();
        hideProgressToast();
        showToast('リモートから更新を取得しました', 'success');
        setTimeout(() => location.reload(), 400);
    });
}
...
function showDiffDialog(labels, keys, onConfirm) {
    ...
    dialog.innerHTML = `
        <h3>リモートに更新があります</h3>
        <p style="font-size:12px;margin-bottom:10px;">以下のデータがリモートで更新されています。取得するものを選択してください。</p>
        ${labels.map((label, i) => `<div class="diff-item"><label><input type="checkbox" checked data-key="${keys[i]}"> ${escapeHtml(label)}</label></div>`).join('')}
        <div class="diff-dialog-actions">
            <button class="sys-btn" id="diff-confirm" style="flex:1;">取得</button>
            <button class="sys-btn" id="diff-skip" style="flex:1;background:var(--input-bg);color:var(--text);">スキップ</button>
        </div>
    `;
    overlay.appendChild(dialog);
    document.body.appendChild(overlay);

    dialog.querySelector('#diff-confirm').onclick = () => {
        const checked = Array.from(dialog.querySelectorAll('input[type="checkbox"]:checked')).map(cb => cb.dataset.key);
        document.body.removeChild(overlay);
        onConfirm(checked);
    };
    dialog.querySelector('#diff-skip').onclick = () => {
        document.body.removeChild(overlay);
    };
    overlay.onclick = (e) => {
...
/* ------------------------------------------------------------------
 *  10. Auto-save on browser close / tab switch
 * ------------------------------------------------------------------ */
let _autoSaveInProgress = false;

function autoSaveSync() {
    // Synchronous fallback for beforeunload - just save to editor state
    try {
        if (editor) state.chapters[state.currentIdx].body = editor.value;
        if (memoArea) {
...
async function autoSaveAsync() {
    ...
    try {
        autoSaveSync();
        state.ghRepo = document.getElementById('gh-repo')?.value?.trim() || state.ghRepo;
        state.deviceName = document.getElementById('device-name')?.value?.trim() || state.deviceName;
        await persistNow();
    } catch (e) { console.error('Auto-save failed:', e); }
    finally { _autoSaveInProgress = false; }
}

// visibilitychange is the most reliable for mobile browsers
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') autoSaveAsync();
});

// pagehide fires on tab/window close
window.addEventListener('pagehide', () => { autoSaveSync(); });

// beforeunload as final fallback
...
async function syncAttachmentsToGithub(headers, owner, repo) {
    ...
    for (const ref of refs) {
        ...
        await fetch(apiUrl, { method: 'PUT', headers, body: JSON.stringify({ message: `asset: ${ref.name}`, content: btoa(binary) }) });
        ref.githubPath = path;
    }
}
...
function getCurrentMemoCloudKey() {
    ...
    if (state.memoScope === 'folder') {
        const b = getCurrentFolderMemoBundle();
        return { scope:'folder', ownerId: (state.currentFolderId && state.currentFolderId !== 'all') ? state.currentFolderId : 'root', memoId:String(b.currentMemoIdx || 0) };
    }
...
async function syncCurrentMemoAttachmentsFromCloud() {
    ...
    const remoteIndex = fromBase64ToJson(res.body.content);
    if (!Array.isArray(remoteIndex?.items)) return;
    state.assetsIndex = { items: remoteIndex.items, updatedAt: remoteIndex.updatedAt || Date.now() };
    const key = getCurrentMemoCloudKey();
    const refs = remoteIndex.items.filter(x => x.scope===key.scope && x.ownerId===key.ownerId && x.memoId===key.memoId);
...
function setAIBusy(nextBusy) {
    ...
    if (aiBusy) {
        ...
        aiThinkingTimer = setInterval(() => { aiThinkingDots = aiThinkingDots >= 3 ? 1 : aiThinkingDots + 1; renderAIChatLog(); renderAIUsage(); }, 420);
    } else {
        clearInterval(aiThinkingTimer); aiThinkingTimer = null; aiThinkingDots = 1;
    }
    ['ai-prompt','ai-proofread-prompt','ai-scope','ai-scope-proofread','ai-send-chat','ai-send-proofread','ai-chat-clear-btn'].forEach(id => {
        ...
        if (el) el.disabled = aiBusy || !navigator.onLine;
    });
...
function addTagToChapter(chapterIdx, tagId) {
    ...
    if (!tags.includes(tagId)) { tags.push(tagId); state.chapters[chapterIdx].tags = tags; }
}
function removeTagFromChapter(chapterIdx, tagId) {
    if (!state.chapters[chapterIdx]) return;
    state.chapters[chapterIdx].tags = (state.chapters[chapterIdx].tags || []).filter(t => t !== tagId);
    if (!state.chapters[chapterIdx].tags.length) state.chapters[chapterIdx].tags = ['root'];
}
function toggleTagOnChapter(chapterIdx, tagId) {
    if (!state.chapters[chapterIdx]) return;
    const tags = state.chapters[chapterIdx].tags || [];
    tags.includes(tagId) ? removeTagFromChapter(chapterIdx, tagId) : addTagToChapter(chapterIdx, tagId);
}
function hasChapterTag(chapterIdx, tagId) { return (state.chapters[chapterIdx]?.tags || []).includes(tagId); }
function getChaptersWithTag(tagId) {
    return state.chapters.map((ch, idx) => ({idx, chapter: ch})).filter(({chapter}) => (chapter.tags || []).includes(tagId)).map(({idx}) => idx);
}
...
function switchChapter(i) {
    const idx = Number(i);
    if (Number.isNaN(idx) || idx < 0 || idx >= state.chapters.length) return;
    save(); state.currentIdx = idx; refreshUI(); loadChapter(idx);
    showToast(`「${state.chapters[idx].title}」へ切替`, 'success');
}
function addChapter() {
    const t = prompt("タイトル:"); if (!t) return; save();
    const tags = state.currentFolderId && state.currentFolderId !== 'all' ? [state.currentFolderId] : ['root'];
    state.chapters.push({title: t, body: "", snapshots: [], memos: [{name:"メモ", content:"", attachments: []}], currentMemoIdx: 0, tags});
    state.currentIdx = state.chapters.length - 1; refreshUI(); loadChapter(state.currentIdx);
}
...
function renameChapter(i) {
    ...
    if (!next) return;
    state.chapters[i].title = next.trim(); refreshUI(); save();
}
function splitChapter() {
    const pos = editor.selectionStart;
    const bodyBefore = editor.value.slice(0, pos);
    const bodyAfter = editor.value.slice(pos);
...
function mergeChapter() {
    if (state.currentIdx === 0 || !confirm("前の話と統合しますか？")) return;
    save(); state.chapters[state.currentIdx - 1].body += "\n" + editor.value;
    state.chapters.splice(state.currentIdx, 1); state.currentIdx--;
...
function openChapterTagEditor(chapterIdx) {
    ...
    if (checkboxesEl) {
        checkboxesEl.innerHTML = state.folders.map(f =>
            `<label style="display:block; margin:8px 0; cursor:pointer;"><input type="checkbox" id="tag-${f.id}" class="tag-checkbox" value="${f.id}" ${(ch.tags || []).includes(f.id) ? 'checked' : ''}> ${f.name}</label>`
        ).join('');
    }
...
function saveTagEditor() {
    ...
    if (!newTags.length) newTags.push('root');
    state.chapters[currentTagEditorChapterIdx].tags = newTags;
    closeTagEditor(); refreshUI(); save();
}
...
function restoreBody(i) {
    if (confirm("本文を復元しますか？")) {
        editor.value = state.chapters[state.currentIdx].snapshots[i].body;
        save(); updateHighlight();
    }
}
...
function renameMemo(i) {
    ...
    if (!name) return;
    target[i].name = name.trim(); renderMemos(); save();
}
...
function deleteMemo(i) {
    const target = state.memoScope === 'local' ? state.chapters[state.currentIdx].memos : state.memoScope === 'folder' ? getCurrentFolderMemoBundle().memos : state.globalMemos;
    if (target.length <= 1) return showToast("最低1つは必要です。", "error");
...
function switchMemo(i) {
    save();
    if (state.memoScope === 'local') state.chapters[state.currentIdx].currentMemoIdx = i;
    else if (state.memoScope === 'folder') getCurrentFolderMemoBundle().currentMemoIdx = i;
    else state.currentGlobalMemoIdx = i;
    renderMemos();
}
function addMemoTab() {
...
function getCurrentMemo() {
    ...
    return ctx.memos[ctx.owner[ctx.idxKey]];
}
...
async function previewMemoAttachment(i) {
    ...
    const closeBtn = `<div style="display:flex;justify-content:space-between;align-items:center;"><strong>${ref.name}</strong><button onclick="closeMemoAttachmentPreview()"><span class="material-icons" style="font-size:16px;">close</span></button></div>`;
    if ((ref.type || '').startsWith('image/')) {
        const url = URL.createObjectURL(stored.data);
        pane.innerHTML = `<div class="config-item" style="display:block;">${closeBtn}<img class="attachment-preview-media" src="${url}"></div>`;
    } else if ((ref.type || '').startsWith('audio/')) {
        const url = URL.createObjectURL(stored.data);
        pane.innerHTML = `<div class="config-item" style="display:block;">${closeBtn}<audio class="attachment-preview-media" controls src="${url}"></audio></div>`;
    } else if ((ref.type || '').startsWith('video/')) {
        const url = URL.createObjectURL(stored.data);
        pane.innerHTML = `<div class="config-item" style="display:block;">${closeBtn}<video class="attachment-preview-media" controls src="${url}"></video></div>`;
    } else {
        const text = typeof stored.data === 'string' ? stored.data : await stored.data.text();
        pane.innerHTML = `<div class="config-item" style="display:block;">${closeBtn}<pre class="preview-text">${escapeHtml(text)}</pre></div>`;
    }
}
...
function getVisibleChapterIndexes() {
    ...
    return state.chapters.map((_, i) => i).filter(i => fid === 'all' || (state.chapters[i].tags || []).includes(fid));
}
function renderFolderFilter() {
    const sel = document.getElementById('folder-filter');
    if (!sel) return;
    sel.innerHTML = [`<option value="all">すべてのタグ</option>`].concat((state.folders || []).map(f => `<option value="${f.id}">${f.name}</option>`)).join('');
...
function addFolder() {
    ...
    state.currentFolderId = id; refreshUI(); save();
}
...
function renameCurrentFolder() {
    ...
    if (!name) return;
    target.name = name.trim(); refreshUI(); save();
}
...
function switchMenuTab(tab) {
    state.menuTab = tab;
    document.querySelectorAll('#menu-tabs .tab').forEach(el => el.classList.toggle('active', el.dataset.tab === tab));
...
function toggleFavoriteAction(key, checked) {
    ...
    if (checked) { if (!list.includes(key)) list.push(key); }
    else { state.favoriteActionKeys = list.filter(x => x !== key); }
...
function extractJapaneseTerms(text) {
    const freq = {};
    const add = token => { const t = token.trim(); if (!t || t.length < 2) return; freq[t] = (freq[t] || 0) + 1; };
    if (typeof Intl !== 'undefined' && Intl.Segmenter) {
        const seg = new Intl.Segmenter('ja', { granularity: 'word' });
        for (const part of seg.segment(text || '')) { if (part.isWordLike) add(part.segment); }
        return freq;
    }
    (text || '').replace(/[\u3000\s]+/g, ' ').split(' ').forEach(add);
    return freq;
}
...
function ensureHighlightMirror() {
    ...
    Object.assign(highlightMirror.style, { position:'absolute', top:'0', left:'-9999px', visibility:'hidden', pointerEvents:'none', whiteSpace:'pre-wrap', wordBreak:'break-word', overflowWrap:'anywhere' });
...
function updateHighlight() {
    ...
    ['fontSize','fontFamily','fontWeight','fontStyle','lineHeight','letterSpacing','paddingTop','paddingRight','paddingBottom','paddingLeft','borderTopWidth','borderRightWidth','borderBottomWidth','borderLeftWidth','boxSizing','textIndent'].forEach(prop => { mirror.style[prop] = style[prop]; });
    ...
    const top = Math.max(0, Math.round(highlightMarker.offsetTop - editor.scrollTop));
    hl.style.display = 'block'; hl.style.height = `${lineHeight}px`; hl.style.top = `${top}px`;
}
...
function getSearchRegex() {
    ...
    if (!q) return null;
    try { return regexMode ? new RegExp(q, 'gu') : new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gu'); }
    catch { showToast('正規表現が不正です', 'error'); return null; }
}
function collectSearchResults() {
    ...
    const hits = [];
    targets.forEach(t => { let m; re.lastIndex = 0; while ((m = re.exec(t.text)) !== null) { hits.push({ chapterIndex:t.i, title:t.title, index:m.index, length:m[0].length, text:m[0] }); if (m.index === re.lastIndex) re.lastIndex++; } });
...
function toggleSearchPanel(forceOpen = false, forceClose = false) {
    ...
    if (p.style.display === 'block') { document.getElementById('search-query').focus(); collectSearchResults(); }
}
function findNext() {
    const hits = collectSearchResults();
    if (!hits.length) return showToast('見つかりませんでした', 'error');
    const all = !!document.getElementById('search-all-chapters')?.checked;
    let hit = hits.find(h => h.chapterIndex === state.currentIdx && h.index >= editor.selectionEnd);
    if (!hit) hit = hits[0];
    if (all && hit.chapterIndex !== state.currentIdx) switchChapter(hit.chapterIndex);
    editor.focus(); editor.setSelectionRange(hit.index, hit.index + hit.length); updateHighlight();
}
function replaceCurrent() {
    const r = document.getElementById('replace-query').value;
    const selected = editor.value.slice(editor.selectionStart, editor.selectionEnd);
    if (!selected) return findNext();
    editor.setRangeText(r, editor.selectionStart, editor.selectionEnd, 'select'); save(); findNext();
}
function replaceAllMatches() {
    ...
    let count = 0;
    if (all) {
        state.chapters.forEach(ch => { ch.body = ch.body.replace(re, () => { count++; return r; }); re.lastIndex = 0; if (state.currentIdx === state.chapters.indexOf(ch)) editor.value = ch.body; });
    } else {
...
function switchAITab(tab) {
    ...
    state.aiTab = mode;
    document.getElementById('ai-tab-chat')?.classList.toggle('active', mode === 'chat');
    document.getElementById('ai-tab-proofread')?.classList.toggle('active', mode === 'proofread');
...
function getAIScopeText() {
    const scope = document.getElementById('ai-scope')?.value || 'chapter';
    if (scope === 'folder') return getVisibleChapterIndexes().map(i => `### ${state.chapters[i].title}\n${state.chapters[i].body || ''}`).join('\n\n');
...
function getValidAppOrigin() {
    ...
    return (!o || o === 'null' || o === 'file://') ? 'https://kakudraft.local' : o;
}

function buildProviderHeaders(provider, key, withJson = true) {
    const headers = {};
    if (withJson) headers['Content-Type'] = 'application/json';
    if (provider === 'google') { headers['x-goog-api-key'] = key; return headers; }
    headers.Authorization = `Bearer ${key}`;
    if (provider === 'openrouter') { headers['HTTP-Referer'] = getValidAppOrigin(); headers['X-Title'] = 'KakuDraft'; }
    return headers;
}
...
async function fetchAIModels() {
    ...
    try {
        ...
        if (provider === 'openrouter') {
            const r = await fetch('https://openrouter.ai/api/v1/models', { headers: buildProviderHeaders('openrouter', key, false) });
            const j = await r.json();
            models = (freeOnly ? (j.data || []).filter(m => Number(m?.pricing?.prompt || 0) === 0 && Number(m?.pricing?.completion || 0) === 0) : (j.data || [])).map(m => m.id);
            ...
            const r = await fetch('https://generativelanguage.googleapis.com/v1beta/models', { headers: buildProviderHeaders('google', key, false) });
            const j = await r.json();
        ...
        showToast('モデル一覧を取得しました', 'success'); save();
    } catch (e) { showToast(`モデル取得失敗: ${e.message}`, 'error'); }
}
...
async function callAI(messages, jsonMode = false) {
    ...
    if (provider === 'google') {
        ...
        const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, { method:'POST', headers: buildProviderHeaders('google', key), body: JSON.stringify(body) });
        const j = await r.json();
...
document.getElementById('ai-free-only')?.addEventListener('change', () => {
    state.aiFreeOnly = !!document.getElementById('ai-free-only')?.checked; queuePersist();
});
...
document.addEventListener("keydown", async e => {
    ...
    switch (key) {
        case "s":
            ...
            if (document.getElementById('gh-token')?.value && state.ghRepo) { clearTimeout(syncTimer); syncTimer = setTimeout(() => githubSync("up"), 1200); }
            break;
...
window.onload = async () => {
    ...
    renderAIChatLog(); renderAIUsage(); setAIBusy(false);
    switchAITab(state.aiTab || 'chat');
    refreshUI(); loadChapter(state.currentIdx);

    // Startup diff check: compare local vs remote per-piece
    if (navigator.onLine && token && state.ghRepo) {
        setTimeout(() => { checkRemoteDiffOnStartup(token).catch(() => {}); }, 400);
    }
};
